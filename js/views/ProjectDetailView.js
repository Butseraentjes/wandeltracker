import { subscribeToProject, saveWalk, subscribeToWalks, setProjectDestination } from '../lib/firebase.js';
...
export class ProjectDetailView extends View {
    ...
    updateProjectHeader(project) {
        ...
        if (header) {
            ...
            if (project.destination) {
                destinationHtml = `
                    <div class="mt-4 p-4 bg-blue-50 rounded-lg">
                        <h3 class="text-lg font-semibold">Einddoel</h3>
                        <p>${project.destination.name}</p>
                        <p class="text-sm text-gray-600">Afstand tot doel wordt berekend...</p>
                    </div>
                `;
            } else {
                destinationHtml = `
                    <div class="mt-4">
                        <button id="set-destination-btn" class="text-blue-600 hover:text-blue-800">
                            + Stel einddoel in
                        </button>
                    </div>
                `;
            ...
            // Event listener voor de "Stel einddoel in" knop
            if (!project.destination) {
                const setDestinationBtn = document.getElementById('set-destination-btn');
                if (setDestinationBtn) {
                    setDestinationBtn.addEventListener('click', () => this.showSetDestinationModal(project));
                }
            }
        }
    }

    showSetDestinationModal(project) {
        const modalHtml = `
            <div id="destination-modal" class="modal">
                <div class="modal-content">
                    <div class="modal-header">
                        <h3>Stel einddoel in</h3>
                        <button class="close-modal">&times;</button>
                    </div>
                    <form id="destination-form">
                        <div class="form-group">
                            <label for="destination-name">Waar wil je naartoe wandelen?</label>
                            <input type="text" id="destination-name" required 
                                   placeholder="Voer een stad of gemeente in (bijv: Gent)">
                            <p class="text-sm text-gray-500 mt-1">
                                Let op: het einddoel kan later niet meer worden gewijzigd
                            </p>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="secondary-btn" id="cancel-destination">
                                Annuleren
                            </button>
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                await setProjectDestination(project.id, destinationName);
                closeModal();
                ...
                if (error.message === "Kon de opgegeven bestemming niet vinden") {
                    ...
                    alert('Er is iets misgegaan bij het instellen van het doel. Probeer het opnieuw.');
                }
            } finally {
...
}`Startlocatie: ${project.location.street} ${project.location.number}, ${project.location.postalCode} ${project.location.city}`);

        // Als er een bestemming is, voeg die toe aan de kaart
        if (project.destination) {
            this.showDestinationOnMap(project.destination);
        }
    }

    showDestinationOnMap(destination) {
        // Verwijder bestaande destination marker als die er is
        if (this.destinationMarker) {
            this.map.removeLayer(this.destinationMarker);
        }

        // Voeg nieuwe marker toe
        this.destinationMarker = L.marker([destination.coordinates.lat, destination.coordinates.lng], {
            icon: L.divIcon({
            ...
            .addTo(this.map)
            .bindPopup(`Einddoel: ${destination.name}`);

        // Pas kaartweergave aan om zowel start als einddoel te tonen
        const bounds = L.latLngBounds([
            [50.981728, 4.127903], // startpunt
            [destination.coordinates.lat, destination.coordinates.lng] // eindpunt
        ]);
    ...
    async updateMapPath(project, totalDistance) {
        ...
        try {
            let routeCoords;
            if (project.destination) {
                // Als er een bestemming is, bereken route naar de bestemming
                const response = await fetch(`https://graphhopper.com/api/1/route?vehicle=foot&locale=nl&key=${config.graphhopper.apiKey}&point=${startLat},${startLng}&point=${project.destination.coordinates.lat},${project.destination.coordinates.lng}&points_encoded=false&instructions=true&elevation=true`);
                const data = await response.json();

                if (data.paths && data.paths.length > 0) {
                    routeCoords = data.paths[0].points.coordinates.map(coord => [coord[1], coord[0]]);
                } else {
                    throw new Error('Geen route gevonden');
                }
            } else {
                // Als er geen bestemming is, gebruik random richting
            ...
            // Voeg huidige positie marker toe
            const currentPos = routeCoords[Math.floor(totalDistance / 100 * routeCoords.length)];
